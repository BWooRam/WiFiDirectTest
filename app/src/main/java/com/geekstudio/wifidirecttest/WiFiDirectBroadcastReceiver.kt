    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (action) {
            ...
            WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION -> {
                ...
                Log.d(TAG, "onReceive WIFI_P2P_STATE_CHANGED_ACTION device = $device, deviceName = ${device?.deviceName}, deviceAddress = ${device?.deviceAddress}")
            }
            ...
            WifiP2pManager.WIFI_P2P_THIS_DEVICE_CHANGED_ACTION -> {
                ...
                val device = intent.getParcelableExtra<WifiP2pDevice>(WifiP2pManager.EXTRA_WIFI_P2P_DEVICE)
                Log.d(TAG, "onReceive WIFI_P2P_THIS_DEVICE_CHANGED_ACTION device = $device, deviceName = ${device?.deviceName}, deviceAddress = ${device?.deviceAddress}")
            }
